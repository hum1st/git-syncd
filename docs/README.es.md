...
1. `git fetch origin`
2. Comparar `HEAD` con upstream (`@{u}` o `origin/<branch>`)
3. Sin retraso → `false` **sin tocar el working tree**
4. Con retraso → fast-forward; si falla y `force: true`, reset/clean y alinear al remoto
...
| `force`  | `boolean` | `true`          | Solo si hace falta actualizar y hay cambios locales: reset/clean y alinear al remoto          |
